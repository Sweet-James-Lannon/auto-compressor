...
def compress_parallel(
    ...
    total_chunks_bytes = sum(p.stat().st_size for p in ordered_paths)
    total_chunks_mb = total_chunks_bytes / (1024 * 1024)

    # Fast path: if we must split the final output, avoid merging the full PDF.
    # Splitting each compressed chunk is much faster than size-based splitting on the merged file.
    if split_threshold_mb and total_chunks_mb > split_threshold_mb:
        from split_pdf import split_by_size

        report_progress(75, "splitting", f"Splitting {len(ordered_paths)} chunks into parts under {split_threshold_mb}MB...")
        final_parts: List[Path] = []
        part_index = 1

        for chunk_idx, chunk_path in enumerate(ordered_paths, 1):
            chunk_size_mb = get_file_size_mb(chunk_path)
            if chunk_size_mb <= split_threshold_mb:
                dest = working_dir / f"{base_name}_part{part_index}.pdf"
            ...
            chunk_base = f"{base_name}_chunk{chunk_idx}"
            sub_parts = split_by_size(chunk_path, working_dir, chunk_base, split_threshold_mb)
